...
{
    case 1:
        IngresarImporte_Curso(ref curso, ref total_un_curso, ref esta);
        if(!esta)
        {
...
void IngresarImporte_Curso(ref string curso, ref int total, ref bool esta)
{
    int regalo= 0;
    curso= IngresarString("¿Curso?"); 
    esta= Recaudaciones_Cursos.ContainsKey(curso);
    if(esta){
        Console.WriteLine("Ya fue ingresado este curso");
    }
    else
    {
